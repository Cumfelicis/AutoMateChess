...
def build_model():
    ...
    decision_input = layers.Input(shape=(DECISION_INPUT_SIZE, ))  # other factors influencing the decision, player strength, remaining time etc.
...
if __name__ == '__main__':
    ...
    for i in range(235):
        ...
        if history.history['val_loss'][-1] < lowest_loss:
            model.save_weights(f'D:/models/bestweights4.h5', save_format='h5')
        ...
        plt.savefig(f'D:/models/figure4{i}.png')
        plt.show(block=False)
        model_json = model.to_json()
        with open(f'D:/models/architecture4{i}.json', 'w') as json_file:
            json_file.write(model_json)
        model.save_weights(f'D:/models/weights4{i}.h5', save_format='h5')
