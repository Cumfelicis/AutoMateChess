...
class Board:
    ...
    def play_move_on_board(self, piece_square, target_square, captured=No(0, 0, 0, 0, 0, 0), castled=False,
        ...
        if real_move:
            if not self.squares[target_square[0]][target_square[1]].name.lower() == 'n':
                ...
                if target_square[1] > piece_square[1]:  # nach rechts
                    if target_square[0] > piece_square[0]: # nach unten
                        if abs(target_square[1] - piece_square[1]) == 1: #nach rechts unten
                            ...
                            self.stepper_y.move(ptrb.get_pos(target_square[0]))
                            self.multistepper.run_to()
                            self.stepper_x.move_to(ptrb.get_pos(target_square[1]))
                            self.stepper_x.run_to()
                        else: # nach rechts rechts unten
                            self.stepper_y.move(-self.between_squares)
                            self.stepper_x.move_to(ptrb.get_pos(target_square[1]))
                            self.multistepper.run_to()
                            self.stepper_y.move_to(ptrb.get_pos(target_square[0]))
    ...
    def set_up(self, fen, first_set_up=False):
        ...
        if self.real_board and not first_set_up:
            # self.setup_real_board(temp, self.squares)  # TODO: reimplement
            del temp
